...
void PointCloudPreprocess::Process(const livox_ros_driver2::msg::CustomMsg::SharedPtr &msg,
    ...
    int plsize = msg->point_num; //! dalin remark:point_num == 0（驱动发空帧/包损坏）则index=-1，导致定位或建图进程直接挂掉
    //todo dalin add:
    //todo const size_t n = std::min<size_t>(msg->points.size(), msg->point_num); if (n == 0) return;
